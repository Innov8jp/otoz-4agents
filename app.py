Of course. Here is the complete, final version of the application code, with a clear marker at the end of the script as you requested.

This single `app.py` file contains all the features we have built, including the sidebar, filters, price graph, and the advanced chatbot that learns from your `intents_200.json` file.

### Final Setup Reminder

For this code to work, please ensure your GitHub repository contains these **four essential files**:

1.  **`app.py`** (The full code below)
2.  **`requirements.txt`** (With `streamlit`, `pandas`, and `altair`)
3.  **`inventory.csv`** (Your vehicle data)
4.  **`intents_200.json`** (Your chatbot training data)
...
```python
# ==============================================================================
# FINAL, ALL-IN-ONE PRODUCTION SCRIPT (v8 - All Features Included)
# ==============================================================================
...
def get_bot_response(user_input: str):
    ...
    if tag == 'update_filter':
        ...
        if year_match:
            ...
            active_filters['year_min'] = year; active_filters['year_max'] = year; updated = True
        if updated:
...
def car_filters(inventory):
    with st.sidebar:
        ...
        with st.form("car_filters_form"):
            ...
            selected_make = st.selectbox("Make", make_list, index=make_list.index(st.session_state.active_filters.get("make", "All")))
            model_list = ["All"]
            if selected_make != "All": model_list += sorted(inventory[inventory['make'] == selected_make]['model'].unique())
            selected_model = st.selectbox("Model", model_list, index=model_list.index(st.session_state.active_filters.get("model", "All")))
            year_min_val, year_max_val = int(inventory['year'].min()), int(inventory['year'].max())
            selected_years = st.slider("Year", year_min_val, year_max_val, st.session_state.active_filters.get("year_range", (year_min_val, year_max_val)))
...
def display_market_data_chart(df, make, model):
    if df.empty or 'make' not in df.columns: return
    history_df = simulate_price_history(df)
    filtered_data = history_df.query("make == @make and model == @model")
...
def main():
    ...
    if st.session_state.offer_placed:
        ...
        display_market_data_chart(inventory, current_car['make'], current_car['model'])
