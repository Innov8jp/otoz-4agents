# ==============================================================================  
# FINAL, ALL-IN-ONE PRODUCTION SCRIPT (UX Fixes for Expander & Dynamic Ports)  
# ==============================================================================  
...
def load_data(path):  
    ...
    if path.endswith('.csv'):  
        df = pd.read_csv(path)  
        df['image_url'] = df.get('image_url', df.apply(lambda r: f"https://placehold.co/600x400?text={r.make}+{r.model}", axis=1))  
...
def generate_invoice_html(cust,car,bd):  
    ...
    return f"""<html><body><h2>Invoice</h2><p>Date: {datetime.now().date()}</p>  
    <p>Customer: {cust['name']} &lt;{cust['email']}&gt;</p><table border='1' cellpadding='5'><tr><th>Item</th><th>Amount</th></tr>{rows}</table></body></html>"""  
...
def get_bot_response(inp):  
    if 'i agree' in inp.lower():  
        ...
        html=generate_invoice_html(st.session_state['customer_info'],car,bd)  
        return html  
...
def user_info_form():  
    with st.sidebar:  
        ...
        countries=[""]+sorted(PORTS_BY_COUNTRY)  
        sel_c=st.selectbox("Country",countries,index=countries.index(ci.get('country','')))  
        ports=PORTS_BY_COUNTRY.get(sel_c,[])  
        port_list=[""]+ports  
        sel_p=st.selectbox("Port of Discharge",port_list,index=port_list.index(ci.get('port_of_discharge','')))  
        if st.button("Save Info",use_container_width=True):  
            st.session_state['customer_info']={  
                'name':name,'email':email,'phone':phone,'country':sel_c,'port_of_discharge':sel_p  
            }  
...
def car_filters(inv):  
    with st.sidebar:  
        ...
        with st.form("filt"):  
            ...
            yrange=(int(inv['year'].min()),int(inv['year'].max()))  
            yr=st.slider("Year",yrange[0],yrange[1],af.get('year_range',yrange))  
...
def display_market_data_chart(hist,make,model):  
    ...
    if df.empty: return  
    # Expander starts collapsed; user can click to expand  
    with st.expander("Show 6-Month Price Trend"):
...
def main():  
    ...
    with c2:
        if st.button("❌ Next Vehicle",use_column_width=True):
