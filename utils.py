...
# --- PDF Generation is disabled, so related code is removed ---
# We will add a more reliable version back later if needed.
...
def load_intents(file_path: str):
    ...
    try:
        ...
        st.error(f"Training file not found at {file_path}. Please make sure it's uploaded.")
        return None
...
def get_bot_response(user_input: str):
    ...
    # --- Step 1: Create a master list of all patterns and a map to their tags ---
    all_patterns = []
    pattern_to_tag = {}
    for intent in intents_data['intents']:
        ...
        for pattern in intent['patterns']:
            ...
            pattern_to_tag[pattern] = tag

    # --- Step 2: Find the best matching pattern for the user's input ---
    # We use `difflib` to find the closest match, which is more flexible than simple keywords
    matches = get_close_matches(user_input, all_patterns, n=1, cutoff=0.6) # cutoff controls similarity
    ...
    if best_match_tag:
        for intent in intents_data['intents']:
            if intent['tag'] == best_match_tag:
                # Pick a random response from the list for that intent
    ...
    if '{' in response and '}' in response:
        ...
        # Look for number in user input to fill in offer amount
