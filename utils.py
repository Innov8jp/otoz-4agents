# utils.py (Final, Simplified Version)
...
def load_intents(file_path: str):
    try:
        ...
        st.error(f"Training file not found: {file_path}. Please upload it to your repository.")
        return None
    except json.JSONDecodeError:
        st.error(f"Error decoding the JSON from {file_path}. Please check its syntax.")
        return None

### UPDATED: This function is now much simpler and faster. ###
@st.cache_data
def load_inventory():
    ...
    try:
        ...
        # Add required columns if they don't exist in the CSV for some reason
        ...
        st.error(f"Fatal Error: `inventory.csv` not found. Please upload it to your repository.")
        return pd.DataFrame()
...
def get_bot_response(user_input: str):
    ...
    matches = get_close_matches(lowered_input, list(pattern_to_tag.keys()), n=1, cutoff=0.6)
